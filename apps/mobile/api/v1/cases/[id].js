...
module.exports = async function handler(req, res) {
  ...
  await withClient(res, async (client) => {
    ...
    const [deadlineResult, actionResult, documentResult, eventResult] = await Promise.all([
      ...
      client.query("select id, created_at::text as date, action as title, coalesce(description, '') as description, coalesce(user_id::text, 'Sistema') as \"user\" from case_events where organization_id = $1 and case_id = $2 order by created_at", [orgId, id])
    ]);
