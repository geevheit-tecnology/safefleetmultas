...
const maxOcrImageBytes = Number(process.env.OCR_MAX_IMAGE_BYTES || 4_500_000);
...
async function handleCaseAction(req, res) {
  ...
  if (body.action === "remote_ocr") return remoteOcr(req, res, body);
  return sendJson(res, 400, { error: "unknown_action" });
}

function permissionForCaseAction(action) {
  if (action === "attach_document" || action === "prepare_extraction" || action === "smart_triage" || action === "confirm_extraction" || action === "remote_ocr") return ACTION_PERMISSIONS.upload_document;
...
async function remoteOcr(req, res, body) {
  ...
  if (Buffer.byteLength(image, "utf8") > maxOcrImageBytes) {
